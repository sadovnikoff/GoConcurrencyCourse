...
const (
	...
	batchFlushingTimeout = "10ms"
	batchSize            = 10
	maxSegmentSize       = "10MB"
	defaultDirPath       = "data/wal"
	loggingLevel         = "debug"
)
...
type WalConfig struct {
	...
	FlushingTimeout string `yaml:"flushing_timeout"`
	SegmentSize     string `yaml:"max_segment_size"`
...
func ParseConfig(reader io.Reader) (*Config, error) {
	...
	if config.Wal != nil {
		if config.Wal.FlushingTimeout == "" {
			config.Wal.FlushingTimeout = batchFlushingTimeout
		}

		if config.Wal.SegmentSize == "" {
			config.Wal.SegmentSize = maxSegmentSize
		}

		if config.Wal.BatchSize == 0 {
			config.Wal.BatchSize = batchSize
		}

		if config.Wal.DirPath == "" {
			config.Wal.DirPath = defaultDirPath
		}
	}
...
func ParseSize(cfgSize string) (int, error) {
